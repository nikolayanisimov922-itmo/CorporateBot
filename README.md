...
- [x] **2. Notion** — выгрузка базы (`python sync_notion.py`)
- [x] **3. Поиск (RAG)** — индекс + отладочный поиск (`build_index.py` / `search_cli.py`)
- [x] **4. База знаний** — Claude отвечает по базе + ссылка на Notion
- [x] **5. Актуальность** — кнопка «Обновить базу» + автообновление по расписанию
- [x] **6. Рассылки** — объявление всем + предпросмотр и отчёт
- [x] **7. Приём данных** → Google Sheets (Заявка / Показания / Обратная связь) ← *мы здесь*
- [ ] 8. Полная проверка
- [ ] 9. Хостинг 24/7
